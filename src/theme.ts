...
export const ColorModeContext = createContext<{ toggleColorMode: () => void } | undefined >(undefined);
...
export const useMode = () => {
  ...
  const colorMode = useMemo(
    () => ({
      // Fonction qui change le mode couleur entre "light" et "dark"
      toggleColorMode: () =>
        setMode((prev) => (prev === "light" ? "dark" : "light")),
    }),
    [] // ✅ `useMemo` évite de recréer cet objet à chaque rendu
  );

  // 🎨 5. Création du thème MUI en fonction du mode sélectionné
  const theme = useMemo(() => createTheme(themeSettings(mode)), [mode]);
  // ✅ `useMemo` empêche la recréation du thème si `mode` ne change pas

  // 📌 6. Retourne le thème et l'objet `colorMode` pour que les composants puissent l'utiliser
